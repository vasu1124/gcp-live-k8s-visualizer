...
function renderGroups(groups) {
    ...
    forProperty(groups, function (key, group) {
...
function connectDeployments(clusterInstance, deployments, pods) {
    forEach(deployments, function (i, deployment) {
        forEach(pods, function (j, pod) {
            if (matchLabelSelectors(pod.metadata.labels, deployment.spec.selector.matchLabels)) {
...
function connectServices(clusterInstance, services, pods) {
    forEach(services, function (i, service) {
        forEach(pods, function (j, pod) {
            if (matchLabelSelectors(pod.metadata.labels, service.spec.selector)) {
                clusterInstance.connect({
